...
import { conversations, type Conversation } from "./growth-data";
...
export function InboxScreen() {
  ...
  const send = () => {
    ...
    showToast(`Reply sent to ${active.name} on ${active.channel}`);
    setDraft("");
  };
        ...
        <div className="bg-surface border border-border rounded-lg overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.map((c) => {
              ...
              return (
                <div key={c.id} onClick={() => setActiveId(c.id)} className="flex gap-2.5 px-3.5 py-3 border-b border-border-faint cursor-pointer" style={{ background: sel ? "var(--bg-content)" : undefined }}>
